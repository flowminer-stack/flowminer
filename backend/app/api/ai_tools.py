"""Structured / tool-style AI endpoints.

These are the "AI tools" endpoints — distinct from the conversational
routes in :mod:`app.api.ai` (chat, chat-suggestions, narrate,
suggest-best-practice). They each take structured input and return
structured output rather than streaming a free-form chat answer:

  1. POST /ai/text-to-bpmn      — Convert a natural-language description
                                  into BPMN XML.
  2. POST /ai/agent/run         — Agentic tool-use loop; the model can
                                  call discover/conformance/insights etc.
                                  as tools until it decides it has enough.
  3. POST /ai/explain-variant   — Compare one variant's cases against the
                                  rest of the log and explain the delta.
  4. POST /ai/explain           — Lightweight 1-2 sentence plain-language
                                  explanation for a single bottleneck /
                                  conformance / prediction row.
  5. POST /ai/extract-log       — Multi-turn event-log extraction copilot
                                  that emits SQL / pandas extraction code.

This router mounts at the SAME prefix (``/api/v1/ai``) as
:mod:`app.api.ai`; FastAPI allows multiple routers per prefix, so the
route paths are byte-identical to before the split.
